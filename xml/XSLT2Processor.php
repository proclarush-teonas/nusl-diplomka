...
class XML_XSLT2Processor
{
     ...
     */
    private static function _url2filepath($url, $base = __DIR__)
    {
        if ($base === null) {
            $base = getcwd();
        }
     ...
     */
    public function __construct($processor = 'SAXON', $processorPath = "/saxon/saxon9.jar", $interface = 'CLI')
    {
        if (preg_match('/^SAXON/i', $processor)) {
            $this->processor = 'SAXON';
            $pv = $this->_substring_between(strtoupper($processor), 'SAXON');
            $this->processorVersion = (empty($pv) ? '9' : $pv);
        ...
        if (preg_match('/^(JAVA|(JAVA\-)?CLI|COM)$/', $interface)) {
            ...
            if (is_string($processorPath) || (is_array($processorPath) && !empty($processorPath['processor']))) {
                $this->processorPath = is_string($processorPath) ? str_replace('\\', '/', dirname(__DIR__)) . $processorPath : $this->_url2filepath($processorPath['processor']);
                //echo $this->processorPath;
                // tady je treba mit zakomentovane radky pokud je to na windowsech, jinak je treba nastavit u saxon9.jar souboru prava k executable
                ...
                //    throw new Exception('Invalid path to XSLT processor binary given to XML_XSLT2Processor::__construct()'. var_dump(is_executable($this->processorPath)) . var_dump(is_file($this->processorPath)), 2);
                //}
    ...
    public function importStylesheet($stylesheet)
    {
        try {
            if ($stylesheet instanceof DOMDocument) {
                $this->stylesheet = tempnam($stylesheetLocation = __DIR__, 'XSL_TEMP');          
    ...
    /**
     *  Pridana funkce kvuli zakazanosti escapeshellarg() funkce v ramci hostingu
     *  mela by delat to same co dela vyse zminena implemenovana funkce php
     */                   
    
    public function escapeshellarg_special($file) 
    {
    ...
    public function transformToURI($xml, $uri)
    {
        ...
        if (!$xml instanceof DOMDocument) {
            ...
            $source = tempnam(__DIR__, 'XML_TEMP');
        ...
        if (preg_match('/CLI$/', $this->interface)) {
            ...
            if ($this->processor === 'SAXON') {            
                ...
                $this->command = ($isJAVACLI ? $this->escapeshellarg_special($this->runtime) . ' -cp ' : null) . $this->escapeshellarg_special(($isJAVACLI ? dirname(__FILE__) . DIRECTORY_SEPARATOR . 'XSLT2Processor.jar' . PATH_SEPARATOR : null) . $this->processorPath) . ($isJAVACLI ? ' XSLT2Processor.saxon' : null) . $commandOptions . ' -o' . ($this->processorVersion != 8 ? ':' : ' ') . $inputs['output'] . ($this->processorVersion != 8 ? ' -s:' : ' ') . $inputs['source'] . ($this->stylesheet !== XML_XSLT2PROCESSOR_PI ? ($this->processorVersion != 8 ? ' -xsl:' : ' ') . $inputs['stylesheet'] : null) . ' ' . $params;
            ...
            // Kvuli windows se musi zmenit \ na / plus se do prikazu dostaly jednoduche uvozovky (') kolem cesty k procesoru, taky se odeberou.
            $this->command = str_replace("\\", "/", $this->command);
            $this->command = str_replace("'", "", $this->command);            
            $processorInstance = proc_open(escapeshellarg($this->command), array(1 => array('pipe', 'w'), 2 => array('pipe', 'w')), $pipes);
